...
const copy = {
  ...
  en: {
    ...
    chatTitle: 'AmazonIA EC', chatStatus: 'Demo · Static responses', chatHello: 'Hello! What kind of property are you looking for?', chatHint: 'Automatic filters will be enabled in the final version.', chatPlaceholder: 'Type your question...', voice: 'Voice reading', voiceHint: 'Enable it and move the cursor over the content.', voiceOn: 'Reading enabled', voiceOff: 'Reading disabled', light: 'Light mode', dark: 'Dark mode', expand: 'Expand image', virtualTour: 'Virtual tour', dragTour: 'Drag to explore the space in 360°', close: 'Close', filters: 'Filters', propertyType: 'Property type', lot: 'Lot', farm: 'Farm', size: 'Size', minimum: 'Minimum', maximum: 'Maximum', province: 'Province', canton: 'County', parish: 'Parish', any: 'All', clearFilters: 'Clear filters', applyFilters: 'Apply filters', noResults: 'No properties match these filters.',
  },
};
...
function PropertyCard({ property, t, lang }) {
  ...
  return <article className="property-card"><div className="property-image"><img src={property.image} alt={title} style={{objectPosition: property.position}}/><span className="property-badge">{property.badge}</span><button className={`favorite ${saved ? 'active' : ''}`} onClick={() => setSaved(!saved)}><Heart fill={saved ? 'currentColor' : 'none'}/></button></div><div className="property-body"><div className="property-meta"><span>{typeLabel}</span><span><MapPin/>{property.location}</span></div><h3>{title}</h3><div className="property-features"><span><LandPlot/>{property.area}</span>{property.beds && <span><BedDouble/>{property.beds}</span>}{property.baths && <span><Bath/>{property.baths}</span>}</div><div className="property-footer"><div><small>{t.from}</small><strong>{property.price}</strong></div><a className="detail-arrow" href={`/propiedades/${property.id}`} aria-label={t.details}><ArrowRight/></a></div><div className="property-card-actions"><VirtualTour property={property} t={t} compact/><a className="advisor-card-button" href="https://wa.me/593000000000"><MessageCircle/>{t.advisor}</a></div></div></article>;
}
...
function PropertyMap({ t }) {
  ...
  useEffect(() => {
    ...
    area.bindPopup(`<strong>Amazonía EC Propiedades</strong><br>${t.mapNote}`).openPopup();
